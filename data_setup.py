...
for site in xi_api['cluster']:	
	...
	if subprocess.call(['ping', param, '1', site['ip']]) == 0:
		...
		# PUSH SQL
		db_import = "ssh -o MACs=hmac-sha1 -l " + site['user'] + " " + site['ip'] + " 'cd ~/var/www/BHT-EMR-API; mysql -uroot -proot openmrs < db/sql/openmrs_metadata_1_7.sql -v -f; mysql -uroot -proot openmrs < db/sql/alternative_drug_names.sql -v -f; mysql -uroot -proot openmrs < db/sql/moh_regimens_v2020.sql -v -f; db/sql/bart2_views_schema_additions.sql -v -f'"
		os.system(db_import)
